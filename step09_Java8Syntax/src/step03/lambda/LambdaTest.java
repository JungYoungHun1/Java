...
//		int method2(int x, int y);
	...
	public static int operation(int x, int y, Calculation cal) {
...
//		li2.method(5);
		...
		// 람다3-1 : 매개변수가 두개일때는 매개변수 괄호는 생략 불가능
		// method2가 있을때에는 에러 발생 -> 어떤 것을 구현하는지 모호, 형태가 달라도 애초에 발생할 수 있는 오류 방지하기 위해 -> 에러
		// 발생 시킴 -> 따라서 함수형 인터페이스는 "하나의 메소드만" 구현하도록 설계
		LambdaInter3 li3;
		li3 = (int x, int y) -> {
			return x + y;
		};
//		System.out.println(li3.method(3, 5));
		...
		System.out.println(operation(5, 5, div));
		...
		// 람다식
		BangMark bangMark = (s1, s2) -> {
			System.out.println(s1 + " " + s2 + "!");
		};
		bangMark.addBangMark(str1, str2);

		// 람다식은 내부적으로 내부 익명 클래스가 적용된것
		BangMark bangMark2 = new BangMark() {
